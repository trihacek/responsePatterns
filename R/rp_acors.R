...
#' @param max.lag An integer. Define the maximum lag for which auto-correlations should be computed (see Details for the default value).
...
rp.acors <- function(data,
  ...
  if(is.null(max.lag)) {
    ...
    max.lag <- 2 * length(levels(as.factor(unlist(data))))
